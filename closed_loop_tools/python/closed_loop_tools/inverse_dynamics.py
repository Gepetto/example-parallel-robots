...
def relaxedClosedLoopInverseDynamicsCasadi(rmodel, cmodels, q, vq, aq, act_matrix, u0, tol=1e-6):
    """
        relaxedClosedLoopInversecDynamicsCasadi(rmodel, cmodels, q, vq, aq, act_matrix)
        Computes the controls and contact forces that solves the following problem.

        min (1/2) || A u + J^T f - tau||^2

        where fwd represents the integrated forward dynamics on a time dt, A represents the actuation matrix, 
        u_0 is a vector of reference controls and J is the contact Jacobian.
    ...
    A_ = np.concatenate((act_matrix, Jac.transpose()), axis=1)
    b_ = pin.rnea(rmodel, data, q, vq, aq)
    P = A_.T@A_
    p = b_.T@A_
    A = np.zeros((1, nx))
    b = np.zeros(1)
