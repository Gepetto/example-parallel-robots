...
import numpy as np
from example_parallel_robots.actuation_model import ActuationData
...
def computeClosedLoopFrameJacobian(model,data,constraint_model,constraint_data,actuation_model,actuation_data,q0,idframe):
    ...
    actuation_data.Jf_closed[:,:]=pin.computeFrameJacobian(model,data,q0,idframe,pin.LOCAL)@actuation_data.dq
...
def inverseConstraintKinematicsSpeed(model,data,constraint_model,constraint_data,actuation_model,actuation_data,q0,ideff,veff):
    ...
    actuation_data.vqfree[:]=-actuation_data.pinvJfree@actuation_data.Jmot@actuation_data.vqmot
    #reorder of vq
