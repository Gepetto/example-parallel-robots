        ...
        for j, name in enumerate(self.rmodel.names):
            ...
            for iv in range(self.rmodel.joints[j].nv):
                ...
                slider = tk.Scale(slider_frame, variable=var, orient=tk.VERTICAL, from_=-3.0, to=3.0,
                                  resolution=0.01,command=partial(self.slider_display,iq))      # When sliders are moved, call this function
...
class OptimizerFrame:
    ...
    def createTargetSliders(self, tkParent, pack=True, scale=1):
        ...
        for i, direction in enumerate(['x', 'y', 'z']):
            ...
            slider_label.pack(side=tk.BOTTOM)
            slider = tk.Scale(slider_frame, variable=var, orient=tk.VERTICAL, from_=-1.0, to=1.0,
                              resolution=0.02*scale, command=self.slider_update)
            slider.pack(side=tk.BOTTOM)
        if pack: frame.pack(side=tk.TOP)
        return(frame)
