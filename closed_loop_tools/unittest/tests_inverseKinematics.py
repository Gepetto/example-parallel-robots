...
from closed_loop_kinematics import closedLoopInverseKinematicsCasadi, closedLoopInverseKinematicsScipy, closedLoopInverseKinematicsProximal
...
if __name__ == "__main__":
    ...
    vizutils.addViewerSphere(viz, contactNameRight, 0.03, [1., .2, .2, .5])
    vizutils.applyViewerConfiguration(viz, contactNameRight, pin.SE3ToXYZQUAT(Mgoal))
    
    # * Inverse Kinematics
    frame_effector = 'bout_pied_frame'

    InvKin = closedLoopInverseKinematicsProximal(model, data, constraint_models, constraint_datas, Mgoal, name_eff=frame_effector, onlytranslation=False, max_it=100, eps=1e-12, rho=1e-5, mu=1e-4)
