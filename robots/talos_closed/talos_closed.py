...
def TalosClosed(closed_loop=True, only_legs=True):
    ...
    if not closed_loop:
        ...
        new_model.names[new_model.getJointId("free_leg_right_5_joint")] = "mot_leg_right_5_joint"
        new_model.names[new_model.getJointId("free_leg_left_5_joint")] = "mot_leg_left_5_joint"
        constraint_models = []
        q0 = pin.neutral(new_model)
    ...
    if only_legs:
        ...
        jointToLockIds = [i for (i, n) in enumerate(new_model.names) if n in jointToLockNames]
        # new_model, [collision_model, visual_model] = pin.buildReducedModel(
        #     new_model,
        #     [collision_model, visual_model],
        #     jointToLockIds,
        #     q0,
        # )
        new_model, constraint_models, actuation_model, visual_model, collision_model = freezeJoints(new_model, constraint_models, actuation_model, visual_model, collision_model, jointToLockIds, q0)

    return(new_model, constraint_models, actuation_model, visual_model, collision_model)
