...
def addXYZAxisToFrames(rm, vm, basename='XYZ', scale=1, world=False):
    ...
    for i, frame in enumerate(rm.frames):
        if i == 0 and not world:
            continue
        vm.addGeometryObject(pin.GeometryObject(
            f'{basename}_{frame.name}', i, pin.SE3.Identity(), hppfcl.Sphere(.001*scale)))
...
def addXYZAxisToJoints(rm, vm, basename='XYZ', scale=1):
    ...
    for i, name in enumerate(rm.names):
        vm.addGeometryObject(pin.GeometryObject(
            f'{basename}_{name}', i, pin.SE3.Identity(), hppfcl.Sphere(.001*scale)))
...
def addXYZAxisToConstraints(rm, vm, cms, basename='XYZ_cst', scale=1):
    ...
    for cm in cms:
        i = cm.joint1_id
        vm.addGeometryObject(pin.GeometryObject(
            f'{basename}_{cm.name}_1', i, cm.joint1_placement, hppfcl.Sphere(.001*scale)))
        i = cm.joint2_id
        vm.addGeometryObject(pin.GeometryObject(
            f'{basename}_{cm.name}_2', i, cm.joint2_placement, hppfcl.Sphere(.001*scale)))
